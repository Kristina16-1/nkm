...
class Edge:
    ...
    # def is_good(self):
    #     return self.gaps[0].beg < 1e-12 and self.gaps[0].fin > -1e-12
...
class Facet:
    ...
    # проверка на полную невидимость
...
class Polyedr:
        ...
        with open(file) as f:
            for i, line in enumerate(f):
                if i == 0:
                    ...
                    # задание самой грани
                    fa = Facet(vertexes)
                    self.facets.append(fa)
                    if fa.right_cen():
                        self.right_facets.append(fa)
    ...
    def part_area(self):
        _part_area = 0
        for f in self.right_facets:
            a = []
            for n in range(len(f.vertexes)):
                a.append(Edge(f.vertexes[n - 1], f.vertexes[n]))
            for e in a:
